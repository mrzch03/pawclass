...
| title             | string                   | Yes      | Scene title, concise and clear                                                                   |
| description       | string                   | Yes      | 1-2 sentences describing teaching purpose                                                        |
| keyPoints         | string[]                 | Yes      | 3-5 core points                                                                                  |
| teachingObjective | string                   | No       | Corresponding learning objective                                                                 |
| estimatedDuration | number                   | No       | Estimated duration (seconds)                                                                     |
| order             | number                   | Yes      | Sort order, starting from 1                                                                      |
| suggestedImageIds | string[]                 | No       | Suggested image IDs to use                                                                       |
| mediaGenerations  | MediaGenerationRequest[] | No       | AI image/video generation requests when PDF images insufficient                                  |
| quizConfig        | object                   | No       | Required for quiz type, contains questionCount/difficulty/questionTypes                          |
| interactiveConfig | object                   | No       | Required for interactive type, contains conceptName/conceptOverview/designIdea/subject           |
| pblConfig         | object                   | No       | Required for pbl type, contains projectTopic/projectDescription/targetSkills/issueCount/language |
