...
- **When dimensions are provided** (e.g., "**img_1**: dimensions: 884x424 (aspect ratio 2.08)"):
...
Wrong -- gap too small, arrow extends into elements:
  ...
  Arrow:  left=330, end=[60,0], width=60 X (width=60 makes a HUGE arrowhead)

Correct -- proper gap and stroke:
  Rect A: left=60, width=250 (right edge = 310)
  Rect B: left=390 (gap = 80px -- room for arrow)
  Arrow:  left=320, start=[0,0], end=[60,0], width=3 OK (thin stroke, arrow within gap)
```
...
1. Start with your `height`, compute `width = height x aspectRatio`.
...
```
shape: left=60, top=150, width=400, height=120
text:  left=60, top=150, width=360, height=76  X NOT CENTERED
```
...
```
shape: left=60, top=150, width=400, height=120
text:  left=60, top=150, width=420, height=130  X OVERFLOWS
```
...
```
shape: left=60, top=150, width=400, height=120
text:  left=80, top=172, width=360, height=76   OK CENTERED
```
...
12. **Text-Background pairs**: For each text with a background shape:
...
13. No unintended element overlaps (especially check LaTeX elements -- their rendered height may be much larger than specified)
14. Image placed near related text (25-35px gap)
